...
function useDateValidation(holidays: string[], maxAdvanceDays: number): (dateStr: string) => ValidationStatus {
  // Obtenemos 'now' y 'today' una sola vez cuando el hook se inicializa
  const now = useMemo(() => dayjs(), []);
  const today = useMemo(() => now.startOf("day"), [now]);
  const maxAllowed = useMemo(() => today.add(maxAdvanceDays, "day"), [today, maxAdvanceDays]);
  ...
  const validateDate = (dateStr: string): ValidationStatus => {
    ...
    if (isToday) {
      ...
      if (now.hour() >= CUTOFF_HOUR) {
         return { selectable: false, reason: `Hora límite superada (${CUTOFF_HOUR}:00 hs)` };
      }
    }

    // 2. Validaciones de fecha
    if (d.isBefore(today)) return { selectable: false, reason: "Fecha pasada" };
    if (d.isAfter(maxAllowed)) return { selectable: false, reason: `Máx ${maxAdvanceDays} días de anticipación` };
    if (d.day() === 1) return { selectable: false, reason: "Lunes cerrado" }; // Monday=1
    if (holidaySet.has(dateStr)) return { selectable: false, reason: "Feriado" };
